...
- Zobraz si `Služby`
- Otevři nastavení služby přes ikonu ozubeného kolečka. První otevření zobrazí představení `OFeedu` s odkazy jak ho začít používat.
...
- Vyplň přihlašovací údaje vložením zkopírovaného odkazu:
  ...
  ![Copy Credentials](/img/tutorials/quickevent/ofeed_qe_credentials.png) 2. Vlož v `QuickEventu`
...
- Zapni zpracování změn z `O Checklistu`, pokud ho používáš
...
- Každé `xx` vteřiny nahraje startovku, výsledky a stáhne změny pokud jsou (a je zapnuté zpracování)
- Pracuje se změnami v databázi
  - editace závodnika
  - přidání závodníka
  - smazání závodníka
  - vyčtení závodníka
- Odesílá pouze změněná data
- Plní sloupec `Corridor` časem označení v `O Checklistu`
- Průběžně ověřuje, že se používají platné přihlašovací údaje
